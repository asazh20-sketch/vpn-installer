...
# ============================================================
# OPTIONAL/BASE: WIREGUARD TOOLS
# ============================================================

echo ""
echo -e "${CYAN}=============================================${NC}"
echo -e "${CYAN}          WIREGUARD INSTALLATION${NC}"
echo -e "${CYAN}=============================================${NC}"
echo ""
...
if command -v wg >/dev/null 2>&1; then
    echo -e "${GREEN}WireGuard installed: $(wg --version | head -1)${NC}"
else
    echo -e "${RED}WireGuard installation failed.${NC}"
    exit 1
fi
echo -e "${GREEN}No inbound WireGuard port is opened; WireGuard is installed only as a local tunnel capability.${NC}"
echo ""
...
XHTTP_LINK="vless://${XHTTP_UUID}@${SERVER_ADDRESS}:${XHTTP_PORT}?encryption=none&type=xhttp&path=$(printf '%s' "$XHTTP_PATH" | sed 's|/|%2F|g')&mode=auto&security=reality&sni=${SNI}&fp=chrome&pbk=${PUBLIC_KEY}&sid=${SHORT_ID}#VLESS-XHTTP-Reality-${SERVER_ADDRESS}"

# A third copy-paste link for the primary VLESS profile using the server IP.
# This is useful if DNS temporarily lags after a reboot/IP change.
VLESS_IP_LINK="vless://${UUID}@${PUBLIC_IP}:${PORT}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${SNI}&fp=chrome&pbk=${PUBLIC_KEY}&sid=${SHORT_ID}&type=tcp&headerType=none#VLESS-Reality-IP-${PUBLIC_IP}"
...
echo ""
echo -e "${YELLOW}Link 1 - VLESS + REALITY (443) via DuckDNS:${NC}"
echo "$VLESS_LINK"
echo ""
echo -e "${YELLOW}Link 2 - VLESS + REALITY + XHTTP (8443) via DuckDNS:${NC}"
echo "$XHTTP_LINK"
echo ""
echo -e "${YELLOW}Link 3 - VLESS + REALITY (443) via current server IP:${NC}"
echo "$VLESS_IP_LINK"
echo ""
...
# ============================================================
# SECOND VLESS: REALITY + XHTTP ON PORT 8443
# ============================================================

echo ""
echo -e "${CYAN}=============================================${NC}"
echo -e "${CYAN}      VLESS + REALITY + XHTTP (8443)${NC}"
echo -e "${CYAN}=============================================${NC}"
echo ""
...
echo -e "${YELLOW}Adding XHTTP inbound on port ${XHTTP_PORT}...${NC}"
...
# Open 8443 locally if UFW is active.
if command -v ufw > /dev/null 2>&1; then
    ufw allow "${XHTTP_PORT}"/tcp > /dev/null 2>&1 || true
fi

echo -e "${YELLOW}Validating Xray configuration with XHTTP...${NC}"
...
if ! systemctl is-active --quiet xray; then
    echo -e "${RED}Xray failed after adding XHTTP. Restoring previous configuration.${NC}"
    cp "${XRAY_CONFIG}.before-xhttp" "$XRAY_CONFIG"
    systemctl restart xray
    exit 1
fi

echo -e "${GREEN}VLESS + REALITY + XHTTP is running on port ${XHTTP_PORT}.${NC}"
echo -e "${GREEN}XHTTP UUID generated.${NC}"
echo ""
