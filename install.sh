...
if ! retry 3 5 install_xray; then
  echo -e "${RED}Failed to install Xray after 3 attempts. Check your network connection and try again.${NC}"
  exit 1
fi
...
retry 3 5 curl -fsSL https://pkg.cloudflareclient.com/pubkey.gpg -o /tmp/cf-warp.gpg
gpg --yes --dearmor --output /usr/share/keyrings/cloudflare-warp-archive-keyring.gpg /tmp/cf-warp.gpg
echo "deb [signed-by=/usr/share/keyrings/cloudflare-warp-archive-keyring.gpg] https://pkg.cloudflareclient.com/ $(lsb_release -cs) main" > /etc/apt/sources.list.d/cloudflare-client.list
retry 3 5 apt update -y > /dev/null 2>&1
retry 3 5 apt install -y cloudflare-warp > /dev/null 2>&1

systemctl enable warp-svc > /dev/null 2>&1
systemctl restart warp-svc

# Wait for the warp daemon to actually be ready to take commands (up to ~20s), instead of guessing a fixed delay
echo -e "${YELLOW}Waiting for WARP daemon to start...${NC}"
WARP_DAEMON_READY=false
for i in $(seq 1 10); do
  if warp-cli status > /dev/null 2>&1; then
    WARP_DAEMON_READY=true
    break
  fi
  sleep 2
done
...
if [ "$WARP_DAEMON_READY" = false ]; then
  echo -e "${RED}WARP daemon never became ready. Skipping WARP setup; --direct will still work fine.${NC}"
else
  echo -e "${YELLOW}Registering and connecting WARP (this can take up to a minute)...${NC}"

  # Retry registration specifically, since it commonly fails on the very first try
  register_warp() {
    warp-cli --accept-tos registration new > /dev/null 2>&1
    warp-cli --accept-tos account > /dev/null 2>&1
  }
  retry 5 4 register_warp || true

  warp-cli --accept-tos mode proxy > /dev/null 2>&1
  warp-cli --accept-tos proxy port 40000 > /dev/null 2>&1
  warp-cli --accept-tos connect > /dev/null 2>&1

  # Wait for WARP to actually report Connected, up to ~60 seconds, re-issuing connect partway through
  for i in $(seq 1 20); do
    if warp-cli status 2>/dev/null | grep -qi "Connected"; then
      WARP_CONNECTED=true
      break
    fi
    # Halfway through, try nudging it again in case the first connect attempt was dropped
    if [ "$i" -eq 10 ]; then
      warp-cli --accept-tos connect > /dev/null 2>&1
    fi
    sleep 3
...
# Poll for up to ~15 seconds instead of assuming 2s is always enough
XRAY_UP=false
for i in $(seq 1 5); do
...
retry 3 5 /etc/duckdns/duck.sh

DUCKDNS_RESULT=$(cat /etc/duckdns/duck.log)
if [ "$DUCKDNS_RESULT" != "OK" ]; then
  echo -e "${RED}DuckDNS update failed after 3 attempts. Response: $DUCKDNS_RESULT${NC}"
